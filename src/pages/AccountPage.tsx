...
const AccountPage = () => {
  ...
  const statusColor: Record<string, string> = {
    pending: "bg-amber-50 text-amber-800", // yellow-ish
    confirmed: "bg-blue-50 text-blue-800", // blue-ish
    shipped: "bg-violet-50 text-violet-800", // purple-ish
    delivered: "bg-green-100 text-green-800", // green-ish
    cancelled: "bg-red-50 text-red-800", // red-ish
  };
  const renderOrders = () => {
    if (ordersLoading) {
      ...
      <div key={order.id} className="bg-white rounded-2xl p-6 border border-border shadow-sm space-y-4 hover:border-primary/20 transition-all">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-mono text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Order #{order.id.slice(0, 8).toUpperCase()}</p>
              <p className="font-sans text-xs text-muted-foreground mt-0.5">{new Date(order.created_at).toLocaleDateString(undefined, { dateStyle: 'long' })}</p>
            </div>
          </div>
          <div className="flex items-center justify-between sm:justify-end gap-6">
            <span className={`text-[10px] font-sans font-bold uppercase tracking-[0.1em] px-3 py-1.5 rounded-full ${statusColor[order.status] || "bg-background text-muted-foreground"}`}>
              {order.status}
            </span>
            <span className="font-sans font-bold text-primary">{formatPrice(Number(order.total))}</span>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-background pt-4">
          <div className="flex flex-wrap gap-x-4 gap-y-2">
            {(order.order_items as any[])?.map((item: any) => (
              <Link
                ...
                className="font-sans text-[11px] font-medium text-muted-foreground hover:text-secondary transition-colors"
              >
          ...
          {order.loyalty_points_earned > 0 && (
            <div className="flex sm:justify-end">
              <p className="font-sans text-[11px] font-bold text-secondary bg-secondary/5 px-3 py-1 rounded-full flex items-center gap-1">
  ...
  return (
    <div className="container-nuria py-12">
      <div className="flex flex-col sm:flex-row items-center sm:items-end justify-between gap-6 mb-12">
        <div>
          <span className="font-sans text-[12px] font-semibold text-secondary uppercase tracking-[0.2em]">Member Portal</span>
          <h1 className="font-display text-3xl sm:text-4xl font-bold text-foreground mt-2">My Account</h1>
          <div className="w-[60px] h-[3px] bg-secondary mt-4 rounded-full" />
          <p className="font-sans text-sm text-muted-foreground mt-4">{user?.email}</p>
        </div>
        <div className="flex items-center gap-6 bg-white p-4 rounded-2xl border border-border shadow-sm">
          <div className="text-right">
            <p className="font-sans text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Loyalty Points</p>
            <p className="font-display text-2xl font-bold text-secondary leading-none mt-1">{profile?.loyalty_points ?? 0} <span className="text-[10px] font-sans font-medium text-muted-foreground tracking-normal">pts</span></p>
          </div>
          <div className="w-px h-10 bg-muted" />
          <button onClick={signOut} className="p-3 bg-background text-muted-foreground hover:text-primary hover:bg-primary/5 rounded-xl transition-all" aria-label="Sign out">
            <LogOut className="w-5 h-5" />
          </button>
        </div>
      </div>

      <div className="flex gap-2 border-b border-border mb-10 overflow-x-auto scrollbar-hide">
        {tabs.map(({ key, label, icon: Icon }) => (
          ...
          >
            <Icon className={`w-4 h-4 ${tab === key ? "text-primary" : "text-muted-foreground"}`} /> {label}
          </button>
        ))}
      </div>
      ...
      {tab === "loyalty" && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-border p-8 space-y-8 shadow-sm">
            <h3 className="font-display text-xl font-bold text-foreground">Profile Information</h3>
            <div className="grid sm:grid-cols-2 gap-8">
              <div>
                <label htmlFor="acc-name" className="block font-sans text-[13px] font-medium text-foreground mb-2">Full Name</label>
                <input id="acc-name" defaultValue={profile?.name || ""} className="w-full px-4 py-3 border border-border rounded-xl text-sm bg-white font-sans focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all" />
              </div>
              <div>
                <label htmlFor="acc-email" className="block font-sans text-[13px] font-medium text-foreground mb-2">Email Address</label>
                <input id="acc-email" value={user?.email || ""} disabled className="w-full px-4 py-3 border border-border rounded-xl text-sm bg-background text-muted-foreground font-sans italic" />
              </div>
              <div>
                <label htmlFor="acc-mpesa" className="block font-sans text-[13px] font-medium text-foreground mb-2">M-Pesa Number</label>
                <input id="acc-mpesa" defaultValue={profile?.phone || ""} className="w-full px-4 py-3 border border-border rounded-xl text-sm bg-white font-sans focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all" placeholder="+254..." />
              </div>
            </div>
            <div className="pt-4">
              <button 
                onClick={() => toast({ title: "Profile updated" })} 
                className="px-8 py-3.5 bg-primary text-white rounded-xl text-sm font-sans font-bold hover:brightness-90 transition-all shadow-lg shadow-primary/10"
              >
                Save Changes
              </button>
            </div>

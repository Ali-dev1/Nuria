...
export const Navbar = () => {
  ...
  const renderDashboardAction = () => {
    if (!user) {
      return (
        <Link to="/vendor">
          <Button className="bg-transparent border border-white text-white hover:bg-white hover:text-primary text-xs font-bold uppercase tracking-wider px-4 transition-colors">
            VENDOR LOGIN
          </Button>
        </Link>
      );
    ...
    if (profile?.role === 'vendor' || profile?.role === 'admin') {
      return (
        <Link to={profile.role === 'admin' ? "/admin" : "/vendor"}>
          <Button className="bg-primary border border-white text-white hover:bg-white hover:text-primary text-xs font-bold uppercase tracking-wider px-4 transition-colors">
            MY DASHBOARD
          </Button>
        </Link>
      );
        ...
        <div className="container-nuria flex items-center justify-between h-20 px-4 md:px-6">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <div className="hidden lg:flex items-center gap-3 mr-2">
              {(!profile || profile.role === 'customer') && (
                <Link to="/vendor/guide">
                  <Button className="bg-secondary hover:bg-secondary/90 text-white border-none text-xs font-bold uppercase tracking-wider px-4">
                    SELL ON NURIA
                  </Button>
                </Link>
              )}
            ...
            <Link to="/cart" className="relative p-2 text-white hover:text-white/80 transition-colors" aria-label="Cart">
              ...
              {totalItems > 0 && (
                <span className="absolute top-0 right-0 w-5 h-5 bg-secondary text-white text-[10px] font-bold rounded-full flex items-center justify-center border-2 border-primary">
                  {totalItems}
                </span>
              )}
            </Link>
            ...
            <form onSubmit={handleSearch} className="container-nuria">
              <div className="relative">
                ...
                <input
                  ...
                  autoFocus
                  className="w-full pl-12 pr-4 py-3 bg-white rounded-full text-sm text-foreground border-none shadow-xl focus:ring-2 focus:ring-secondary"
                />
              </div>
            </form>
          </div>
